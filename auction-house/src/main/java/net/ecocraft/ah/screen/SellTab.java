...
public class SellTab extends BaseWidget {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (priceValue > 0 && !selected.isEmpty()) {
            ...
            var curr = parent.getCurrency();
            drawSummaryLine(graphics, font, Component.translatable("ecocraft_ah.sell.unit_price_label").getString(), CurrencyFormatter.format(unitPrice, curr), labelX, valueX, panelY + 4, THEME.textGrey, THEME.textLight);
            drawSummaryLine(graphics, font, Component.translatable("ecocraft_ah.sell.total_price_label").getString(), CurrencyFormatter.format(totalPrice, curr), labelX, valueX, panelY + 16, THEME.textLight, THEME.accent);
            drawSummaryLine(graphics, font, Component.translatable("ecocraft_ah.sell.tax_label", Math.round(activeTaxRate * 100)).getString(), "-" + CurrencyFormatter.format(tax, curr), labelX, valueX, panelY + 28, THEME.textGrey, THEME.danger);
            drawSummaryLine(graphics, font, Component.translatable("ecocraft_ah.sell.deposit_label", Math.round(activeDepositRate * 100)).getString(), "-" + CurrencyFormatter.format(deposit, curr), labelX, valueX, panelY + 40, THEME.textGrey, THEME.warning);
            DrawUtils.drawAccentSeparator(graphics, panelX + 4, panelY + 51, panelW - 8, THEME);
            drawSummaryLine(graphics, font, Component.translatable("ecocraft_ah.sell.net_label").getString(), CurrencyFormatter.format(net, curr), labelX, valueX, panelY + 56, THEME.textLight, THEME.success);
        } else {
            DrawUtils.drawPanel(graphics, panelX, panelY, panelW, panelH, THEME);
