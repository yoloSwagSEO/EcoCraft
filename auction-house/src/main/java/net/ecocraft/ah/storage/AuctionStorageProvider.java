...
public class AuctionStorageProvider {
    ...
    public void createAHInstance(AHInstance ah) {
        try (PreparedStatement ps = connection.prepareStatement(
                "INSERT INTO ah_instances (id, slug, name, sale_rate, deposit_rate, durations, allow_buyout, allow_auction, tax_recipient, override_perm_tax, delivery_delay_purchase, delivery_delay_expired) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")) {
    ...
    public void updateAHInstance(AHInstance ah) {
        try (PreparedStatement ps = connection.prepareStatement(
                "UPDATE ah_instances SET slug = ?, name = ?, sale_rate = ?, deposit_rate = ?, durations = ?, allow_buyout = ?, allow_auction = ?, tax_recipient = ?, override_perm_tax = ?, delivery_delay_purchase = ?, delivery_delay_expired = ? WHERE id = ?")) {
            ...
            ps.setInt(10, ah.deliveryDelayPurchase());
            ps.setInt(11, ah.deliveryDelayExpired());
            ps.setString(12, ah.id());
            ps.executeUpdate();
        } catch (SQLException e) {
