...
/**
 * A stationary NPC exchanger. Uses Mob (not PathfinderMob) to avoid
 * pathfinding-related issues. Invulnerable, no gravity, no collision.
 */
public class ExchangerEntity extends Mob {

    private static final EntityDataAccessor<CompoundTag> DATA_SKIN_PROFILE =
            SynchedEntityData.defineId(ExchangerEntity.class, EntityDataSerializers.COMPOUND_TAG);

    private String skinPlayerName = "";

    public ExchangerEntity(EntityType<? extends Mob> type, Level level) {
        super(type, level);
        this.setNoGravity(true);
        this.setInvulnerable(true);
        this.setCustomName(Component.literal("\u00a76Changeur"));
